...
public class CouponController {
    ...
    public ResponseEntity<Void> save(Member member, @RequestBody CouponCreateRequest couponCreateRequest) {
        Long savedId = couponService.save(MemberDto.from(member), couponCreateRequest);
    ...
    public ResponseEntity<List<CouponResponse>> findByMemberId(Member member) {
        List<CouponResponse> coupons = couponService.findByMemberId(MemberDto.from(member));
        return ResponseEntity.ok(coupons);
    }

    @GetMapping("/coupons")
    public ResponseEntity<List<CouponIssuableResponse>> findAll(Member member) {
        List<CouponIssuableResponse> coupons = couponService.findAll(MemberDto.from(member));
        return ResponseEntity.ok(coupons);
    }
}

...
public class OrderDao {
    ...
    public Optional<OrderEntity> findByIdAndMemberId(final Long orderId, final Long memberId) {
        String sql = "SELECT * FROM orders WHERE id = ? AND member_id = ?";

        try {
            return Optional.ofNullable(jdbcTemplate.queryForObject(sql, orderEntityRowMapper, orderId, memberId));
        } catch (EmptyResultDataAccessException e) {
            return Optional.empty();
        }
    }

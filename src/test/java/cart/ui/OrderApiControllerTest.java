...
class OrderApiControllerTest extends ControllerTestConfig {
    ...
    void 장바구니에_담겨있는_상품을_주문한다() {
        ...
        장바구니_주문(회원, 주문할_상품들_id);
    }

    @Test
    void 식별자로_주문_상세내역을_조회한다() {
        final Product 상품 = 상품_등록(피자.PRODUCT);
        final Product 상품2 = 상품_등록(치킨.PRODUCT);
        final Member 회원 = 회원_등록();
        final CartItem 장바구니_상품 = 장바구니_등록(상품, 회원);
        ...
        ExtractableResponse<Response> 장바구니_주문_결과 = 장바구니_주문(회원, 주문할_상품들_id);

        String location = 장바구니_주문_결과.header("Location");
        given(spec)
                .log().all()
                .contentType(APPLICATION_JSON_VALUE)
                .when()
    ...
    private ExtractableResponse<Response> 장바구니_주문(Member 회원, List<Long> 주문할_상품들_id) {
        return given(spec)
                .log().all()
                .filter(document(DOCUMENT_IDENTIFIER,
                        requestHeaders(

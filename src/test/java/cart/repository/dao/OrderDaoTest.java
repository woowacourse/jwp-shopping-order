...
class OrderDaoTest {
    ...
    void 주문_ID로_주문을_조회한다() {
        ...
        int 사용한_포인트 = 10000;
        OrderEntity 주문 = new OrderEntity(null, 멤버.getId(), 사용한_포인트, ID가_있는_상품.getPrice() / 10, 3000, null, null);
    ...
    void 멤버_ID로_모든_주문을_조회한다() {
        ...
        OrderEntity 첫번째_주문 = new OrderEntity(null, 멤버.getId(), 10000, 첫번째_상품.getPrice() / 10, 3000, null, null);
        Long 저장된_첫번째_주문_ID = orderDao.save(첫번째_주문);
        orderProductDao.save(new OrderProductEntity(null, 저장된_첫번째_주문_ID, 첫번째_상품, 10));

        OrderEntity 두번째_주문 = new OrderEntity(null, 멤버.getId(), 10000, 첫번째_상품.getPrice() / 10, 3000, null, null);

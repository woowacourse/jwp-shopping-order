...
public class OrderIntegrationTest extends IntegrationTest {
    ...
    void setUp(final RestDocumentationContextProvider restDocumentation) {
        ...
        this.productId3 = this.createProduct(
    ...
    void getOrders() {
        ...
        final List<OrderResponse> responseBody = response.body().jsonPath()
            .getList(".", OrderResponse.class);

        final List<OrderResponse> expected = List.of(
            new OrderResponse(orderId, null, this.orderItems));
    ...
    void getOrderDetail() {
        ...
        final ExtractableResponse<Response> response = given(this.spec).log().all()
            .filter(document("get-order-detail",
                ...
                responseFields(
                    ...
                    fieldWithPath("paymentAmount.discounts[].discountAmount").description(
                        "정책별 할인 금액"),
        ...
        final OrderDetailResponse orderDetailResponseBody = response.body()
        ...
        Assertions.assertAll(
            () -> assertThat(response.statusCode()).isEqualTo(200),
            () -> assertThat(orderDetailResponseBody).usingRecursiveComparison()
    ...
    private Long requestAddCartItemAndGetId(final Member member, final Long productId) {
        final ExtractableResponse<Response> response = this.requestAddCartItem(member,
            new CartItemRequest(productId));
        return this.getIdFromCreatedResponse(response);
    }


    private ExtractableResponse<Response> requestUpdateCartItemQuantity(final Member member,
        final Long cartItemId, final int quantity) {
        final CartItemQuantityUpdateRequest quantityUpdateRequest = new CartItemQuantityUpdateRequest(
            quantity);
        return given(this.spec).log().all()
            .filter(

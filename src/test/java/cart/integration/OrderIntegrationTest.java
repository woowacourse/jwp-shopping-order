...
class OrderIntegrationTest extends IntegrationTest {
    ...
    void 장바구니에_담겨있는_상품을_주문할_수_있다() {
        // given
        final CreateOrderRequest createOrderRequest = new CreateOrderRequest(List.of(1L, 2L));

        // when
        final var result = 상품_주문(member, createOrderRequest);

        // then
        assertThat(result.statusCode()).isEqualTo(HttpStatus.CREATED.value());
    }

    private ExtractableResponse<Response> 상품_주문(final Member member, final CreateOrderRequest createOrderRequest) {
    ...
    void id로_주문_상세정보를_확인할_수_있다() {
        // given
        final ExtractableResponse<Response> response = 상품_주문(member, new CreateOrderRequest(List.of(1L)));
    ...
    void 사용자의_모든_주문_정보를_확인할_수_있다() {
        // given
        상품_주문(member, new CreateOrderRequest(List.of(1L)));
        상품_주문(member, new CreateOrderRequest(List.of(2L)));

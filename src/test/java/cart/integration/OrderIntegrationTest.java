...
public class OrderIntegrationTest extends IntegrationTest {
    ...
    class 주문을_할_때 {
        ...
        @Test
        void 정상_주문한다() {
            var 응답 = 주문_요청(인증용_밀리, 3000, 밀리_쿠폰_10퍼센트.getId(), 30000, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId());
        ...
        void 다른_사용자_정보로_요청시_실패한다() {
            var 응답 = 주문_요청(인증용_박스터, 3000, 밀리_쿠폰_10퍼센트.getId(), 30000, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId());
        ...
        void 요청한_상품의_총_금액이_계산한_금액과_다르면_실패한다() {
            var 응답 = 주문_요청(인증용_밀리, 3000, 밀리_쿠폰_10퍼센트.getId(), 33001, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId());
        ...
        void 쿠폰을_사용하지_않으면_조건에_맞는_쿠폰을_발급한다() {
            주문_요청(인증용_밀리, 3000, -1L, 33000, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId());

            쿠폰_전체_조회_응답_검증(쿠폰_조회_요청(인증용_밀리), 쿠폰_전체_조회_응답(
    ...
    class 주문을_상세_조회_할_때 {
        ...
        @Test
        void 정상_조회한다() {
            Long 주문_ID = 아이디를_반환한다(주문_요청(인증용_밀리, 3000, 밀리_쿠폰_10퍼센트.getId(), 30000, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId()));

            var 응답 = 주문_상세_조회_요청(인증용_밀리, 주문_ID);
        ...
        void 다른_사용자가_요청_시_실패한다() {
            ...
            var 응답 = 주문_상세_조회_요청(인증용_박스터, 주문_ID);
    ...
    class 사용자_주문을_전체_조회할_때 {
        ...
        void 정상_조회한다() {
            Long 첫번째_주문_ID = 아이디를_반환한다(
                    주문_요청(인증용_밀리, 3000, 밀리_쿠폰_10퍼센트.getId(), 30000, 장바구니_밀리_치킨.getId(), 장바구니_밀리_피자.getId()));
            Long 두번째_주문_ID = 아이디를_반환한다(주문_요청(인증용_밀리, 3000, -1L, 10000, 장바구니_밀리_햄버거.getId()));

            var 응답 = 사용자_주문_전체_조회_요청(인증용_밀리);

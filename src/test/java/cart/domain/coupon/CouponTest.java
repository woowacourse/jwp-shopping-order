...
class CouponTest {
    ...
    void 최소_주문_금액을_만족하면_할인금액을_할인한다() {
        // given
        final Coupon coupon = new Coupon(1L, "50000원 이상 5000원 할인 쿠폰", new PricePolicy(), BigDecimal.valueOf(5000L), new Money(50000));

        // when
        final Money discount = coupon.discount(new Money(50000L));

        // then
        assertThat(discount.getValue().longValue()).isEqualTo(5000);
    }
    ...
    void 최소_주문_금액을_만족하지_않으면_할인_금액은_0원이다() {
        ...
        // then
        assertThat(discount.getValue().longValue()).isEqualTo(0);
    }

    @Test
    void 최소_주문_금액을_만족하면_배달비를_할인한다() {
        // given
        final Coupon coupon = new Coupon(1L, "30000원 이상 배달비 무료 쿠폰", new DeliveryPolicy(), BigDecimal.ZERO, new Money(30000L));
    ...
    void 최소_주문_금액을_만족하지_않으면_배달비를_할인하지_않는다() {
        ...
        // then
        assertThat(discount.getValue().longValue()).isEqualTo(0L);
    }
}
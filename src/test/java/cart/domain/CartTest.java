...
class CartTest {
    ...
    void validateOwner_fail() {
        ...
        Cart cart = new Cart(List.of(
            new CartItem(1L, Quantity.from(1), new Product("치킨", Money.from(10000), "chickenImg"), owner),
            new CartItem(2L, Quantity.from(1), new Product("치킨", Money.from(10000), "chickenImg"), other)
        ));

        // when, then
        assertThatThrownBy(() -> cart.validateOwner(owner))
            .isInstanceOf(CartUnauthorizedException.class);
    }
}
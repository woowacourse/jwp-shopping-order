...
class OrdersControllerTest {
    ...
    void 주문을_완료한다() throws Exception {
        ...
        final OrderSaveRequest orderSaveRequest = new OrderSaveRequest(
                List.of(
                        new OrderItemIdDto(savedCartItem1.getId()),
                        new OrderItemIdDto(savedCartItem2.getId())
                ),
                null);
        ...
        assertThat(findCartItems).hasSize(0);
    ...
    void 주문을_전체_조회한다() throws Exception {
        ...
        final List<CartItem> result = cartItemRepository.findAllByMemberId(member.getId());
        final Orders orders = new Orders(null, member, List.of(
        ...
        mockMvc.perform(get("/orders")
                ...
                .andExpect(jsonPath("$[0].id", is(savedOrdersId.intValue())))
                .andExpect(jsonPath("$[0].totalItemsPrice", is((int) orders.getTotalItemPrice())))
                .andExpect(jsonPath("$[0].discountPrice", is(0)))
                .andExpect(jsonPath("$[0].deliveryFee", is(orders.getDeliveryFee().intValue())))
                ...
                .andExpect(jsonPath("$[0].orderItems[1].name", is(product2.getName())))
                .andExpect(jsonPath("$[0].orderItems[1].imageUrl", is(product2.getImageUrl())))
                .andExpect(jsonPath("$[0].orderItems[1].price", is((int) product2.getPrice())))
                .andDo(print())
                .andReturn();
    }
    ...
    void 주문을_상세_조회한다() throws Exception {
        ...
        final List<CartItem> result = cartItemRepository.findAllByMemberId(member.getId());
        final Orders orders1 = new Orders(null, member, List.of(
                new OrderItem(product1.getName(), product1.getPrice(), product1.getImageUrl(), 2))
        );
        final Orders orders2 = new Orders(null, member, List.of(
                new OrderItem(product2.getName(), product2.getPrice(), product2.getImageUrl(), 1))
        );
        Long savedOrdersId1 = orderRepository.save(orders1).getId();
        Long savedOrdersId2 = orderRepository.save(orders2).getId();
        // when
        mockMvc.perform(get("/orders/" + savedOrdersId1)
                        .header("Authorization", header)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(savedOrdersId1.intValue())))
                .andExpect(jsonPath("$.totalItemsPrice", is((int) orders1.getTotalItemPrice())))
                .andExpect(jsonPath("$.discountPrice", is(0)))
                .andExpect(jsonPath("$.deliveryFee", is(orders1.getDeliveryFee().intValue())))

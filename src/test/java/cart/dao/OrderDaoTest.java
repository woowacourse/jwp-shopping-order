...
class OrderDaoTest {
    ...
    void 주문을_저장한다() {
        ...
        OrderEntity orderEntity = new OrderEntity(MEMBER.getId(), 3000, 3000);
    ...
    void 멤버_아이디를_통해_모든_주문을_찾는다() {
        // given
        insertMember(MEMBER, jdbcTemplate);
        orderDao.save(new OrderEntity(MEMBER.getId(), 3000, 3000));
        orderDao.save(new OrderEntity(MEMBER.getId(), 3000, 3000));
        orderDao.save(new OrderEntity(MEMBER.getId(), 3000, 3000));

        insertMember(new Member(MEMBER.getId() + 1, "email@naver.com", "password"), jdbcTemplate);
        orderDao.save(new OrderEntity(MEMBER.getId() + 1, 3000, 3000));
        orderDao.save(new OrderEntity(MEMBER.getId() + 1, 3000, 3000));

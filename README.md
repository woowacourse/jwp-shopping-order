...
- [ ] 다른 외부 서비스에서 Order를 필요로 한다면?
  - 서비스간 의존?
    - 이를 선택할 경우 순환참조의 여지가 생길 수 있다고 한다. 그래서 서비스 계층 내에서도 계층을 갖도록 하는 내용이 보인다.
    - 근데 .. 계층을 딱 나눌 수가 있는 것인가? 도메인 서비스가 매우 많아진다면 A 서비스가 어떤 문맥에서는 이쪽 계층에 속하다가 또 다른 문맥에서는 저쪽 계층에 속할 수도 있는 거 아닌가?.. 뭔가 머릿속에서 잘 그려지지 않는다.
    - 어쨌든, 계층이 나눠진다면 그때는 의존 방향이 어떻게 이뤄지도록 하면 좋은 것일까? 한 방향으로만 이뤄지도록?? 이렇게 했을 때의 이점은 뭘까.. 아직 고민이 필요하다.
  - 외부 서비스에서 Dao 호출? 
- [x] Member의 equals(), hashcode() 고민
